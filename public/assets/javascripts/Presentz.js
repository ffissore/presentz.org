...
(function() {
  ...
  Agenda = (function() {
    ...
    Agenda.prototype.build = function(presentation) {
      var agenda, chapter, chapterIndex, slideIndex, title, totalDuration, widths, _i, _j, _k, _len, _ref, _ref1, _ref2;
      totalDuration = 0;
      ...
      agenda = '';
      for (chapterIndex = _j = 0, _ref1 = widths.length - 1; 0 <= _ref1 ? _j <= _ref1 : _j >= _ref1; chapterIndex = 0 <= _ref1 ? ++_j : --_j) {
        for (slideIndex = _k = 0, _ref2 = widths[chapterIndex].length - 1; 0 <= _ref2 ? _k <= _ref2 : _k >= _ref2; slideIndex = 0 <= _ref2 ? ++_k : --_k) {
    ...
    Agenda.prototype.select = function(presentation, chapterIndex, slideIndex) {
      var currentSlideIndex, index, _i, _ref;
      $("#" + this.agendaContainer + " div.agendaselected").removeClass("agendaselected");
      currentSlideIndex = slideIndex;
      for (index = _i = 0, _ref = chapterIndex - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; index = 0 <= _ref ? ++_i : --_i) {
        if (chapterIndex - 1 >= 0) {
    ...
    computeBarWidths = function(duration, maxWidth, chapters) {
      var chapter, chapterIndex, clength, slideIndex, slideWidth, slideWidthSum, slides, widths, _i, _j, _len, _ref;
      widths = new Array();
      chapterIndex = 0;
      for (_i = 0, _len = chapters.length; _i < _len; _i++) {
        ...
        for (slideIndex = _j = 1, _ref = slides.length - 1; 1 <= _ref ? _j <= _ref : _j >= _ref; slideIndex = 1 <= _ref ? ++_j : --_j) {
          if (!(slides.length > 1)) {
            continue;
          }
