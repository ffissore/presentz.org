...
(function() {
  "use strict";
  var BlipTv, Html5Video, IFrameSlide, ImgSlide, Kaltura, NoSlide, Presentz, RvlIO, SlideShare, SlideShareOEmbed, SpeakerDeck, SwfSlide, Video, Vimeo, Youtube, root,
  ...
  Kaltura = (function() {
    var urlToKalturaParams;

    function Kaltura(presentz, videoContainer, width, height) {
      this.presentz = presentz;
      this.videoContainer = videoContainer;
      this.width = width;
      this.height = height;
      this.playerPlayEnd = __bind(this.playerPlayEnd, this);
      this.playerUpdatePlayhead = __bind(this.playerUpdatePlayhead, this);
      this.handleEvent = __bind(this.handleEvent, this);
      this.video = new Video(["playing"], ["paused"], ["finish"], this.presentz);
      this.currentTimeInSeconds = 0.0;
      this.kalturaPlayerStateChangedFunctionName = this.presentz.newElementName("kalturaPlayerStateChange");
      this.kalturaPlayerUpdatePlayheadFunctionName = this.presentz.newElementName("kalturaPlayerUpdatePlayhead");
      this.kalturaPlayerPlayEndFunctionName = this.presentz.newElementName("kalturaPlayerPlayEnd");
      this.kalturaKDPEmptyFunctionName = this.presentz.newElementName("kalturaKDPEmpty");
      this.elementId = this.presentz.newElementName();
      this.player = null;
      this.playerLoaded = false;
      this.videoEntryId = null;
      if (typeof window !== "undefined" && window !== null) {
        window[this.kalturaPlayerStateChangedFunctionName] = this.handleEvent;
        window[this.kalturaPlayerUpdatePlayheadFunctionName] = this.playerUpdatePlayhead;
        window[this.kalturaPlayerPlayEndFunctionName] = this.playerPlayEnd;
      }
    }

    Kaltura.prototype.ensureKalturaPlayerIsLoaded = function(videoParams, callback) {
      var $scripts, firstScriptTag, script, url, waitUntilKalturaPlayerIsLoaded,
        _this = this;
      url = "http://cdnapi.kaltura.com/p/" + videoParams.partnerId + "/sp/" + videoParams.partnerId + "00/embedIframeJs/uiconf_id/" + videoParams.uiconfId + "/partner_id/" + videoParams.partnerId;
      if (jQuery("script[src=\"" + url + "\"]").length === 0) {
        window.jsCallbackReady = function(objectId) {
          _this.player = $("#" + objectId)[0];
          _this.player.addJsListener("kdpEmpty", _this.kalturaKDPEmptyFunctionName);
          _this.player.addJsListener("playerStateChange", _this.kalturaPlayerStateChangedFunctionName);
          _this.player.addJsListener("playerUpdatePlayhead", _this.kalturaPlayerUpdatePlayheadFunctionName);
          return _this.player.addJsListener("playerPlayEnd", _this.kalturaPlayerPlayEndFunctionName);
        };
        waitUntilKalturaPlayerIsLoaded = function() {
          if (!((window.kWidget != null) && (typeof mw !== "undefined" && mw !== null))) {
            return;
          }
          clearInterval(_this.timeoutOfWaitUntilKalturaPlayerIsLoaded);
          _this.playerLoaded = true;
          mw.setConfig("EmbedPlayer.RewriteSelector", false);
          return callback();
        };
        this.timeoutOfWaitUntilKalturaPlayerIsLoaded = setInterval(waitUntilKalturaPlayerIsLoaded, 100);
        script = document.createElement("script");
        script.type = "text/javascript";
        script.src = url;
        $scripts = jQuery("script");
    ...
    urlToKalturaParams = function(videoData) {
      var params, parts;
      parts = videoData.url.split("/");
      params = {
        partnerId: parts[parts.length - 3],
        uiconfId: parts[parts.length - 2],
        entryId: parts[parts.length - 1]
      };
      return params;
    };

    Kaltura.prototype.changeVideo = function(videoData, wouldPlay) {
      var videoParams,
        _this = this;
      this.wouldPlay = wouldPlay;
      videoParams = urlToKalturaParams(videoData);
      return this.ensureKalturaPlayerIsLoaded(videoParams, function() {
        if (_this.videoEntryId != null) {
          _this.videoEntryId = videoParams.entryId;
          return _this.player.sendNotification("changeMedia", {
            "entryId": videoParams.entryId
          });
        } else {
          if (jQuery("#" + _this.elementId).length === 0) {
            jQuery("" + _this.videoContainer).append("<div id=\"" + _this.elementId + "\"></div>");
          }
          return kWidget.embed({
            targetId: _this.elementId,
            cache_st: new Date().getTime(),
            wid: "_" + videoParams.partnerId,
            uiconf_id: videoParams.uiconfId,
            flashvars: {
              externalInterfaceDisabled: false,
              autoPlay: _this.wouldPlay
            },
            params: {
              wmode: "opaque"
            },
            entry_id: videoParams.entryId,
            width: _this.width,
            height: _this.height
          });
        }
    ...
    Kaltura.prototype.handle = function(video) {
      return video.url.toLowerCase().indexOf("//kaltura.com/") !== -1;
    };

    Kaltura.prototype.handleEvent = function(event) {
      this.video.handleEvent(event);
    };

    Kaltura.prototype.playerUpdatePlayhead = function(currentTimeInSeconds) {
      this.currentTimeInSeconds = currentTimeInSeconds;
    };

    Kaltura.prototype.playerPlayEnd = function() {
      this.video.handleEvent(this.video.finishStates[0]);
    };

    Kaltura.prototype.currentTime = function() {
      return this.currentTimeInSeconds;
    };

    Kaltura.prototype.skipTo = function(time, wouldPlay) {
      if (wouldPlay == null) {
        wouldPlay = false;
      }
      if (this.player) {
    ...
    Kaltura.prototype.play = function() {
      return this.player.sendNotification("doPlay");
    };

    Kaltura.prototype.pause = function() {
      return this.player.sendNotification("doPause");
    };

    Kaltura.prototype.isPaused = function() {
      return this.video.isInPauseState;
    };
  ...
  RvlIO = (function(_super) {
    ...
    function RvlIO() {
      RvlIO.__super__.constructor.apply(this, arguments);
    }
  ...
  Presentz = (function() {
    ...
    function Presentz(videoContainer, videoWxH, slideContainer, slideWxH) {
      ...
      this.availableVideoPlugins = {
        ...
        kaltura: new Kaltura(this, videoContainer, sizeOfVideo.width, sizeOfVideo.height)
      };
      ...
      this.videoPlugins = [this.availableVideoPlugins.vimeo, this.availableVideoPlugins.youtube, this.availableVideoPlugins.bliptv, this.availableVideoPlugins.kaltura];
    ...
    Presentz.prototype.startTimeChecker = function() {
      ...
      this.interval = setInterval(timeChecker, 200);
    };
